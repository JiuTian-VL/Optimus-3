...
def get_tokenizer(model_name, use_fast: bool = True):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "/data7/Users/xyq/developer/MinecraftOptimus/models/MineStudio_STEVE-1.official/mineclip", use_fast=use_fast
    )
    return tokenizer


def tokenize_batch(texts: str | list[str] | list[list[int]], max_length: int, language_model="clip"):
    """
    Args:
    ...
    if language_model == "clip":
        assert max_length <= 77, "clip language model supports max length of 77 (including BOS and EOS tokens)"
        max_length = 77
