...
import minestudio.utils.mineclip_lib.utils as U
...
class VisionTransformer(nn.Module):
    ...
    def resize_pos_embed(self, new_resolution):
        ...
        for r in new_resolution:
            assert r % self._patch_size == 0, f"{new_resolution} is not divisible by {self._patch_size}"
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.cls_token.repeat((B, 1, 1)), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.pos_embed
        x = self.ln_pre(x)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[ResidualAttentionBlock(width, heads, attn_mask=self.build_attention_mask()) for _ in range(layers)]
        )
    ...
    def forward(self, text):
        ...
        assert x.size(1) <= self.context_length, f"{x.size(1)} exceeds context length {self.context_length}"
