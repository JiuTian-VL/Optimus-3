...
from minestudio.utils.vpt_lib.misc import transpose

from minestudio.utils.factory import (
    build_backbone,
...
class ScalableMinecraftPolicy(nn.Module):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        self.condition_embedding_layer = (
            build_condition_embedding_layer(hidsize=hidsize, **self.condition_embedding)
            if self.condition_embedding
            else None
        )

        self.conditioning_fusion = conditioning_fusion
        self.conditioning_fusion_layer = (
            build_conditioning_fusion_layer(
                hidsize=hidsize,
                **self.conditioning_fusion,
            )
            if self.conditioning_fusion
            else None
        )

        self.action_embedding = action_embedding
        self.action_embedding_layer = (
            ActionEmbedding(num_channels=hidsize, action_space=action_space, **self.action_embedding)
            if self.action_embedding
            else None
        )

        self.past_obs_fusion = past_obs_fusion
        self.past_obs_fusion_layer = (
            PastObsFusion(
                hidsize=hidsize,
                **self.past_obs_fusion,
            )
            if self.past_obs_fusion
            else None
        )
    ...
    def forward(self, obs, state_in, context, ice_latent=None):
        ...
        if ice_latent is None:
            if self.condition_before_vision:
                ce_latent = self.condition_embedding_layer(texts=obs.get("text"), device=obs["img"].device)
        ...
        if ice_latent is None:
            if getattr(self, "condition_embedding_layer", None) and not self.condition_before_vision:
                goal_kwargs = {
