...
from minestudio.utils.vpt_lib.serialize import serialize_object
...
class EnvWorker(mp.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.verbose:
                print(
                    "[worker {}] aveg idle time: {:.2f}ms; aveg working time: {:.2f}ms".format(
                        mp.current_process().name,
                        total_wait_time / wait_count * 1000,
                        total_occupied_time / (occupacy_count + 1e-8) * 1000,
                    )
                )
            ...
            if command == "reset":
                ...
                if self.record:
                    ...
                    if (
                        ...
                        and (self.always_record or cum_reward > 0.0)
                        and (info["accomplishments"][-1] not in ignores)
                    ):
