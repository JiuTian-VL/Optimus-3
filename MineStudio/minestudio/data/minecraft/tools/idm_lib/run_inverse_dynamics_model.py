...
def main(model, weights, video_path, json_path, n_batches, n_frames):
    ...
    for _ in range(n_batches):
        ...
        for _ in range(n_frames):
            ...
            if not ret:
                break
            assert frame.shape[0] == required_resolution[1] and frame.shape[1] == required_resolution[0], (
        ...
        for i in range(n_frames):
            ...
            cv2.putText(frame, f"name: prediction (true)", (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--jsonl-path", type=str, required=False, help="Path to a .jsonl file (Minecraft recording).")
    parser.add_argument("--n-frames", type=int, default=128, help="Number of frames to process at a time.")
    parser.add_argument(
        "--n-batches", type=int, default=10, help="Number of batches (n-frames) to process for visualization."
    )
