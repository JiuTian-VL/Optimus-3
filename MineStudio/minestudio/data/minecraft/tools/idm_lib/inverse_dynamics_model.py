...
class IDMAgent:
    ...
    def __init__(self, idm_net_kwargs, pi_head_kwargs, device=None):
        ...
        idm_policy_kwargs = dict(
            idm_net_kwargs=idm_net_kwargs, pi_head_kwargs=pi_head_kwargs, action_space=action_space
        )
    ...
    def _agent_action_to_env(self, agent_action):
        ...
        action = {"buttons": agent_action["buttons"].cpu().numpy(), "camera": agent_action["camera"].cpu().numpy()}
    ...
    def predict_actions(self, video_frames):
        ...
        predicted_actions, self.hidden_state, _ = self.policy.predict(
            agent_input, first=dummy_first, state_in=self.hidden_state, deterministic=True
        )
