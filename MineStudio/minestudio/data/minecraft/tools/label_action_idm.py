        ...
        while opening > 0:
            for worker_idx in range(self.nb_worker):
                ...
                if command == "free":
                    ...
                    if task is not None:
                        console.log(
                            f"Remain: {len(self.task_list)} | Fetch task: {task} and Assign to worker {worker_idx}."
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--redis-key", type=str, default="none", help="redis key that specifies videos to be processed"
    )
    parser.add_argument("--nb-worker", type=int, default=1, help="number of paralle workers")
    parser.add_argument("--model", type=str, required=True, help="the IDM model path")
    parser.add_argument("--weights", type=str, required=True, help="the IDM weights path")
    parser.add_argument("--video-dir", type=str, required=True, help="the directory containing all the videos")
    parser.add_argument("--save-dir", type=str, required=True, help="the directory saving the predicted actions")
