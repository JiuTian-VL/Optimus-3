...
mc_constants_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mc_constants.1.16.json")
...
ALL_CRAFTING_TABLE_ITEMS = [
    ...
    if item["type"] in CRAFTING_RECIPES_BY_OUTPUT.keys()
    and len(CRAFTING_RECIPES_BY_OUTPUT[item["type"]]) > 0
    and any([recipe["recipeSize"] <= 9 for recipe in CRAFTING_RECIPES_BY_OUTPUT[item["type"]]])
]
...
ALL_SMELTING_ITEMS = [
    ...
    item["type"]
    for item in all_data["items"]
    if item["type"] in SMELTING_RECIPES_BY_OUTPUT.keys() and len(SMELTING_RECIPES_BY_OUTPUT[item["type"]]) > 0
]
...
BEST_ITEMS_PER_EQUIPMENT_SLOT = {
    equip: [item["type"] for item in all_data["items"] if item["bestEquipmentSlot"] == equip]
