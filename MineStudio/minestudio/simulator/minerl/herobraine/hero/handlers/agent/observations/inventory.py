...
class FlatInventoryObservation(TranslationHandler):
    ...
    def from_universal(self, obs):
        ...
        try:
            if (
                obs["slots"]["gui"]["type"] == "class net.minecraft.inventory.ContainerPlayer"
                or obs["slots"]["gui"]["type"] == "class net.minecraft.inventory.ContainerWorkbench"
            ):
...
class PlainInventoryObservation(TranslationHandler):
    ...
    def __init__(self):
        ...
        space = spaces.Dict(
            {
                slot_id: spaces.Dict({"type": spaces.Text(shape=()), "quantity": spaces.Box(low=0, high=64, shape=())})
