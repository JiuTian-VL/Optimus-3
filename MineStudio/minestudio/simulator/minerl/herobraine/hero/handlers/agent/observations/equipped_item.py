...
from minestudio.simulator.minerl.herobraine.hero.mc import EQUIPMENT_SLOTS
    ...
    def __or__(self, other):
        ...
        if isinstance(other, _TypeObservation):
            return _TypeObservation(
                self._keys, list(set(self._items + other._items)), _other=self._other, _default=self._default
            )
        else:
