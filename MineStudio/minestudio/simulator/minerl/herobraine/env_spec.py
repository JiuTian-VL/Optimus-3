...
MISSION_TEMPLATE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hero", "mission.xml.j2")
...
class EnvSpec(abc.ABC):
    ...
    def create_observation_space(self):
        return self._singlify(
            spaces.Dict(
                {agent: spaces.Dict({o.to_string(): o.space for o in self.observables}) for agent in self.agent_names}
            )
        )

    def create_action_space(self):
        action_space = self._singlify(
            spaces.Dict(
                {agent: spaces.Dict({a.to_string(): a.space for a in self.actionables}) for agent in self.agent_names}
            )
        )
        return action_space
    ...
    def create_monitor_space(self):
        return self._singlify(
            spaces.Dict(
                {agent: spaces.Dict({m.to_string(): m.space for m in self.monitors}) for agent in self.agent_names}
            )
        )
    ...
    def _entry_point(self, fake: bool) -> str:
        if fake:
            ...
            return EnvSpec.U_SINGLE_AGENT_ENTRYPOINT if self.is_single_agent else EnvSpec.U_MULTI_AGENT_ENTRYPOINT
    ...
    def __repr__(self):
        ...
        return "{}-{}-spaces({},{})".format(
            self.__class__.__name__, self.name, self.observation_space, self.action_space
        )
