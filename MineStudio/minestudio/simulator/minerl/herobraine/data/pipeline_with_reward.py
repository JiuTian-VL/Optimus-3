    ...
    def batch_iter(self, batch_size):
        ...
        while not map_promise.ready() or not self.data_queue.empty() or not self.random_queue.empty():
            ...
            if incr > self.size_to_dequeue:
                ...
                observation_batch = [
                    HandlerCollection({o: np.asarray(traj_ob[i]) for i, o in enumerate(self.observables)})
                    for traj_ob in traj_obs
                ]
                action_batch = [
                    HandlerCollection({a: np.asarray(traj_act[i]) for i, a in enumerate(self.actionables)})
                    for traj_act in traj_acts
                ]
                mission_handler_batch = [
                    HandlerCollection({m: np.asarray(traj_handler[i]) for i, m in enumerate(self.mission_handlers)})
                    for traj_handler in traj_handlers
                ]
                next_observation_batch = [
                    HandlerCollection({o: np.asarray(traj_n_ob[i]) for i, o in enumerate(self.observables)})
                            ...
                            if frames_queue.full():
                                ...
                                batches.append(
                                    (
                                        ...
                                        mission,
                                        next_obs,
                                        DataPipelineWithReward._calculate_discount_rew(rewards[-nsteps:], gamma),
                                        frame_num + 1 - nsteps,
                                    )
                                )
