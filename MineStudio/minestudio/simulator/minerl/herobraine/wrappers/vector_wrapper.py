...
from minestudio.simulator.minerl.herobraine.wrappers.util import union_spaces, flatten_spaces, intersect_space
...
class Vectorized(EnvWrapper):
    ...
    def __init__(self, env_to_wrap: EnvSpec, common_envs=None):
        assert env_to_wrap.is_single_agent, "Vectorized (currently) only supports single agents environments."
        ...
        self.common_action_space = spaces.Dict({hdl.to_string(): hdl.space for hdl in self.common_actions})
        self.common_observations = reduce(union_spaces, [env.observables for env in self.common_envs])
        self.common_observation_space = spaces.Dict({hdl.to_string(): hdl.space for hdl in self.common_observations})
