...
from minestudio.simulator.minerl.herobraine.hero import handlers as H, mc
from minestudio.simulator.minerl.herobraine.hero.mc import ALL_ITEMS, INVERSE_KEYMAP, SIMPLE_KEYBOARD_ACTION
...
class HumanControlEnvSpec(EnvSpec, ABC):
    ...
    def create_actionables(self) -> List[TranslationHandler]:
        ...
        not all.
        """
        return [H.KeybasedCommandAction(v, v) for v in mc.KEYMAP.values()] + [H.CameraAction()]
...
class SimpleHumanEmbodimentEnvSpec(HumanControlEnvSpec):
    ...
    def create_actionables(self) -> List[TranslationHandler]:
        ...
        """
        return (
            [H.KeybasedCommandAction(k, v) for k, v in INVERSE_KEYMAP.items() if k in SIMPLE_KEYBOARD_ACTION]
            + [
                H.CameraAction(),
                H.CraftAction(["none"] + mc.ALL_PERSONAL_CRAFTING_ITEMS, _other="none", _default="none"),
                H.CraftNearbyAction(["none"] + mc.ALL_CRAFTING_TABLE_ITEMS, _other="none", _default="none"),
            ]
            + [
                H.EquipAction(["none"] + mc.ALL_ITEMS, _other="none", _default="none"),
                H.PlaceBlock(["none"] + mc.ALL_ITEMS, _other="none", _default="none"),
            ]
            + [H.ChatAction()]
