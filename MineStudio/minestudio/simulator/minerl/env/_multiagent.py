...
from minestudio.simulator.minerl.env.malmo import (
    InstanceManager,
    MinecraftInstance,
...
class _MultiAgentEnv(gym.Env):
    ...
    def _process_action(self, actor_name, action_in) -> str:
        ...
        act_space = (
            bottom_env_spec.action_space[actor_name] if self.task.agent_count > 1 else (bottom_env_spec.action_space)
        )
    ...
    def step(self, actions) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, float], bool, Dict[str, Dict[str, Any]]]:
        if not self.done:
            ...
            except (socket.timeout, socket.error, TypeError):
                # If the socket times out some how! We need to catch this and reset the environment.
    ...
    def reset(self) -> Any:
        ...
        while not database_manager.add_resetting_env(request_uuid, MAX_RESETTING_ENV_COUNT):
            if not has_warned:
                rich.print(
                    f"[yellow]WARNING: There are currently too many environments (>= {MAX_RESETTING_ENV_COUNT}) resetting. The reset will block until one finishes. Please enlarge the MAX_RESETTING_ENV_COUNT in _multiagent.py if this is a problem.[/yellow]"
                )
                has_warned = True
            time.sleep(0.1)
        ...
        try:
            ...
            self.done = False
            self.has_finished = dict.fromkeys(self.task.agent_names, False)
    ...
    def _setup_slave_master_connection_info(self, slave_xml: etree.Element, mc_server_ip: str, mc_server_port: str):
    ...
    def _send_mission(self, instance: MinecraftInstance, mission_xml_etree: etree.Element, token_in: str) -> None:
        ...
        while ok != 1:
            ...
            token = (
                token_in + ":" + str(self.task.agent_count) + ":" + str(True).lower()  # synchronous
            )
    ...
    def _TO_MOVE_handle_frozen_minecraft(self, instance):
        if instance.had_to_clean:
            ...
            logger.error("Connection with Minecraft client {} cleaned more than once; restarting.".format(instance))

            instance.kill()
            instance = self._get_new_instance(instance_id=self.instances[0].instance_id)
        else:
