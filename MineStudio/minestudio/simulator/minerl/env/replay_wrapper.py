...
from minestudio.simulator.minerl.herobraine.hero import mc, handlers
...
class MinecraftReplayWrapper(ReplayWrapper):
    ...
    def __init__(
        self, env, replay_file, clip_stats=True, max_steps=None, gui_camera_scaler=1.0, replay_on_reset=False
    ):
    ...
    def replay2env(self, replay_action, next_action):
        ...
        ac = mc.minerec_to_minerl_action(
            replay_action, next_action=next_action, gui_camera_scaler=self.gui_camera_scaler, esc_to_inventory=False
        )
        if self.multiagent:
            ac = {"agent_0": ac}
        return ac
