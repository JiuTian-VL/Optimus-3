...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Temporary directories which should be deleted were the processes to terminate.",
        nargs="+",
    )
...
def launch(parent_pid, child_pid, *temp_dirs):
    ...
    subprocess.check_call(
        [
            sys.executable,
            os.path.join(os.path.dirname(__file__), "process_watcher.py"),
            str(parent_pid),
            str(child_pid),
            "--{}".format(CHILD_DIR_ARG),
        ]
...
def main(args):
    ...
    logger.info(
        "Process watcher started between parent {}".format(args.parent_pid) + " and child {} ".format(args.child_pid)
    )
    time.sleep(1)
    try:
            ...
            if not parent.is_running() or parent is None:
                ...
                if child is not None:
                    ...
                    try:
                        ...
                        logger.warning("Failed to delete temporary child directory. It may have already been removed.")
                    except KeyError:
...
if __name__ == "__main__":
    ...
    coloredlogs.install(
        level=logging.DEBUG, stream=open(os.path.join(os_cur_dir, MINERL_WATCHERS_DIR, watcher_name + ".log"), "w")
    )
