...
SEGMENT_MAPPING = {"Hunt": 0, "Use": 3, "Mine": 2, "Interact": 3, "Craft": 4, "Switch": 5, "Approach": 6}
...
def make_video_fn(session, make_video, save_video, progress=gr.Progress()):
    ...
    return (
        session,
        gr.Button("Make Video", visible=False),
        gr.DownloadButton("Download!", value=filepath, visible=True),
    )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                sam_choice.select(
                    fn=choose_sam_fn,
                    inputs=[sam_choice, rocket_session],
                    outputs=[rocket_session],
                    show_progress=False,
                )
                ...
                with gr.Group():
                    ...
                    clear_points_btn.click(
                        clear_points_fn,
                        inputs=[rocket_session],
                        outputs=[display_image, rocket_session],
                        show_progress=True,
                    )
                ...
                with gr.Group():
                    ...
                    track_flag.select(
                        ...
                        inputs=[track_flag, rocket_session],
                        outputs=[rocket_session],
                        show_progress=False,
                    )
                    with gr.Group(), gr.Row():
                        ...
                        clear_segment_btn.click(
                            clear_segment_fn,
                            inputs=[rocket_session],
                            outputs=[display_image, track_flag, rocket_session],
                            show_progress=True,
                        )

            display_image.select(
                get_points_with_draw,
                inputs=[display_image, add_or_remove, rocket_session],
                outputs=[display_image, rocket_session],
            )
            segment_type.select(
                set_segment_type, inputs=[segment_type, rocket_session], outputs=[rocket_session], show_progress=False
            )
        ...
        with gr.Row():
            with gr.Group():
                ...
                reset_btn.click(
                    fn=reset_fn,
                    inputs=[env_name, rocket_session],
                    outputs=[display_image, rocket_session],
                    show_progress=True,
                )
            ...
            with gr.Group():
                ...
                clear_states_btn.click(
                    fn=clear_memory_fn,
                    inputs=rocket_session,
                    outputs=[display_image, memory_length, rocket_session],
                    show_progress=False,
                )
            ...
            make_video_btn.click(
                make_video_fn,
                inputs=[rocket_session, make_video_btn, save_video_btn],
                outputs=[rocket_session, make_video_btn, save_video_btn],
                show_progress=False,
            )
        ...
        with gr.Row():
            with gr.Group():
                ...
                molmo_btn.click(
                    molmo_fn,
                    inputs=[molmo_text, molmo_session, rocket_session, display_image],
                    outputs=[output_text, display_image],
                    show_progress=False,
                )
