...
def main(cfg: DictConfig):
    ...
    if rollout_manager is None:
        if cfg.detach_rollout_manager:
            rollout_manager = RolloutManager.options(name="rollout_manager", lifetime="detached").remote(
    ...
    training_session = TrainingSession.options(name="training_session").remote(
        hyperparams=cfg, logger_config=cfg.logger_config
    )  # type: ignore
    ...
    trainer = registry.get_trainer_class(cfg.trainer_name)(
        rollout_manager=rollout_manager, model_spec=cfg.model, env_spec=cfg.environment, **cfg.train_config
    )
